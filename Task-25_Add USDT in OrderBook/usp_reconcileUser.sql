USE [cnx]
GO
/****** Object:  StoredProcedure [dbo].[usp_reconcileUser]    Script Date: 2017/12/21 17:08:52 ******/
SET ANSI_NULLS ON
GO
...
select @BTCBalance = @BTCBalance - isnull(sum(A_Amount + A_Commission),0) from [Transaction]  T with(nolock)  
left join OrderBook O1 with(nolock) on T.A_OrderId = O1.OrderId 
where O1.CustomerId = @CustomerId and O1.OrderTypeId = 1 and WantAssetTypeId = 12 and offerAssetTypeid = 2
...
select @ETHBalance = @ETHBalance + ISNULL(sum(A_Amount), 0) from [Transaction] T with(nolock)
left join OrderBook O1 with(nolock) on T.B_OrderId = O1.OrderId 
where O1.CustomerId = @CustomerId and O1.OrderTypeId = 2 and WantAssetTypeId = 5 and offerAssetTypeid = 9

--ETH_CABS
select @ETHBalance = @ETHBalance - isnull(sum(A_Amount + A_Commission),0) from [Transaction]  T with(nolock)  
left join OrderBook O1 with(nolock) on T.A_OrderId = O1.OrderId 
where O1.CustomerId = @CustomerId and O1.OrderTypeId = 1 and WantAssetTypeId = 12 and offerAssetTypeid = 5
...
left join OrderBook O1 with(nolock) on T.A_OrderId = O1.OrderId 
where O1.CustomerId = @CustomerId and O1.OrderTypeId = 1 and WantAssetTypeId = 12 and offerAssetTypeid = 6
...
left join OrderBook O1 with(nolock) on T.A_OrderId = O1.OrderId 
where O1.CustomerId = @CustomerId and O1.OrderTypeId = 1 and WantAssetTypeId = 12 and offerAssetTypeid = 9
...
-- Deposit btc
select @FCABSBalance = @FCABSBalance + isnull(sum(Amount),0) from TransferRequest with (nolock)
