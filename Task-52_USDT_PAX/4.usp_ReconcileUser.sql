USE [cnx]
GO
/****** Object:  StoredProcedure [dbo].[usp_reconcileUser]    Script Date: 2018/11/01 16:58:37 ******/
SET ANSI_NULLS ON
GO
...
@SLRMBalance decimal(18, 8), @PAXBalance decimal(18, 8)
...
@UsedWAX decimal(18, 8),@UsedXLM decimal(18, 8), @UsedTRIA decimal(18, 8), @UsedBEZ decimal(18, 8), @UsedSLRM decimal(18, 8), @UsedPAX decimal(18, 8)
----------------------------------------------------------------------------------------------------------------------------------------------
-- CNY
...
----------------------------------------------------------------------------------------------------------------------------------------------
--PAX
select @PAXBalance= isnull(SUM(Value),0) from Voucher with(nolock) where RedeemedByCustomerId = @CustomerId and AssetTypeId = 39
select @UsedPAX = isnull(SUM(value),0) from Voucher with(nolock) where VoucherStatusId = 3 and IssuedByCustomerId = @CustomerId and AssetTypeId = 39
...
where Customerid = @CustomerId and RequestTypeId = 70 and RequestStatusId = 3
--withdrawal
select @PAXBalance = @PAXBalance - isnull(sum(Amount),0) - isnull(sum(Commision),0) from TransferRequest with (nolock)
where CustomerId = @CustomerId and RequestTypeId = 71 and RequestStatusId = 3
--buy
select @PAXBalance = @PAXBalance  + isnull(sum(B_Amount),0) from [Transaction]  T with(nolock)  
left join OrderBook O with(nolock) on T.A_OrderId = O.OrderId 
where O.CustomerId = @CustomerId and O.OrderTypeId = 1 and WantAssetTypeId = 39 
--sell
select @PAXBalance = @PAXBalance - isnull(sum(B_Amount + B_Commission),0) from [Transaction]  T with(nolock)  
left join OrderBook O with(nolock) on T.B_OrderId = O.OrderId 
...
@CurrentSLRM decimal(18, 8), @CurrentPAX decimal(18, 8)
